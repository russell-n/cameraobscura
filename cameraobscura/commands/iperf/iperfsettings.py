...
class IperfBaseSettings(object):
    """
...
class IperfGeneralSettings(IperfBaseSettings):
    ...
    def output(self, filename):
        ...
        :raise: CameraobscuraError if filename has white-space
        """
        self._output = filename
        if not IperfConstants.no_whitespace_expression.search(str(filename)):
            raise CameraobscuraError("filename must be string of non-whitespace characters, not {0}".format(filename))
