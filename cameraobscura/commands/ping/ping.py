...
# third-party
from theape.parts.connections.clientbase import handlesocketerrors
...
class Ping(object):
    ...
    def operating_system(self):
        ...
        if self._operating_system is None:
            ...
            if self._operating_system not in PingConstants.known_operating_systems:
                raise CameraobscuraError("unknown operating-system: {0}".format(self._operating_system))
            ...
            for line in stderr:
                try:
                    ...
                    if len(line):
                        raise CameraobscuraError("Error getting operating system, try adding it to the config file instead")
    ...
    def check_rep(self):
        ...
        :raise: CameraobscuraError on error
        """
        if self.operating_system not in PingConstants.known_operating_systems:
            raise CameraobscuraError("Unknown Operating System: {0} (known: {1})".format(self.operating_system,
